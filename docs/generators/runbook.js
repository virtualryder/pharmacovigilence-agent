...
const body = [
  ...
  bullet([bold("runtime/ "), "— the Runtime agent (self-locating helper scripts; runs from a fresh clone): ", code("agent.py"), ", ", code("Dockerfile"), ", ", code("requirements.txt"), ", and the ", code("_configure/_launch/_invoke/_obs_setup"), " helper scripts."]),
  ...
  ...codeBlock(["bash _invoke.sh reviewer", "bash _invoke.sh outsider $PV_OUTSIDER_PW   # passwords via env; defaults ChangeMe-*1!"]),
  ...
  table(["Setting", "Where", "Default / value"], [
    ...
    ["Cognito users", "deploy_identity.sh", "reviewer / approver / outsider; passwords via PV_REVIEWER_PW / PV_APPROVER_PW / PV_OUTSIDER_PW (placeholder defaults ChangeMe-*1!)"],
  ...
  ...codeBlock(["bash scripts/destroy_spine.sh              # spine only; leaves identity + Runtime", "# optional, full cleanup:", "bash scripts/destroy_identity.sh           # removes the Cognito pool", "./.venv/Scripts/agentcore.exe destroy      # from runtime/, removes the Runtime", "#   (on macOS/Linux: ./.venv/bin/agentcore destroy)"]),
  ...
  bullet([bold("Portable helper scripts: "), "the ", code("runtime/"), " helpers self-locate (no hardcoded paths), read ", code("spine-state.env"), " relative to the repo, and detect the venv layout per OS (", code("Scripts/"), " vs ", code("bin/"), "), so they run from a fresh clone on any OS. Override via env: ", code("AWS_REGION"), ", ", code("PV_REVIEWER_PW"), "/", code("PV_APPROVER_PW"), "/", code("PV_OUTSIDER_PW"), ", ", code("PV_SPINE_STATE"), "."]),
  spacer(),
